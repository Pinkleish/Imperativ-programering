...
def dungeon_inner_walls(array):
    walls_placed = 0
    while walls_placed != walls:
        height_random = random.randint(2,height-3)
        length_random = random.randint(2,length-3)
        # Randomly assign if the randomly placed wall should expand backwards or forwards and then assign horizontal/vertical
        wall_expand_random = random.randint(-1,1)
        wall_direction_random = random.randint(1,2)
        wall_expand_times_random = random.randint()
        if wall_direction_random % 2 == 0:
...
def dungeon_traps(array):
    ...
    while traps_placed != traps:
        ...
        length_random = random.randint(1,length-2)
        adjacent_empty = 0
        # Makes sure the selected coordinate isn't a treasure
        if array[height_random][length_random] == " ":
            for i in range(height_random-1,height_random+2):
                for j in range(length_random-1,length_random+2):
